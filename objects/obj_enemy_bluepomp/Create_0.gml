...
state_waiting = function() {#region
	...
	sprite_index = spr_bluepomp_idle
...
state_walk = function() {#region
	...
	enemy_ai_walk(spr_bluepomp_turn,spr_bluepomp_walk)
...
state_kick_player = function() {#region
	...
	if sprite_index != spr_bluepomp_kick
	{
		image_speed_set(1)
		attack_dir = point_direction(x,y,obj_player.x,obj_player.y)
		attack_in_done = false
		sprite_index	= spr_bluepomp_kick
		image_index		= 0
	}
...
state_attack = function() {#region
	...
	if sprite_index != spr_bluepomp_attack
	{
		image_speed_set(1)
		attack_dir = point_direction(x,y,target_x,target_y)
		attack_in_done = false
		sprite_index	= spr_bluepomp_attack
		image_index		= 0
	}
	
	if image_index > 3 && !attack_in_done
	{
		audio_play_sound(sound_enemy_weapon_swing,999,0)
...
state_hit = function() {#region
	...
	if in_air
	{
		...
		sprite_index = spr_bluepomp_hurtfly
		image_speed_set(0)
	}
	else
	{
		...
		{
			sprite_index = spr_bluepomp_fall
			image_speed_set(1)
		...
		else
		{
			...
			image_speed_set(1)
			if sprite_index != spr_bluepomp_hurtground
			{
				...
				image_speed_set(1)
				sprite_index	= spr_bluepomp_hurtground
				image_index		= 0
			}
		}
	}
...
state_knockdown = function() {#region
	...
	hspd = lerp(hspd,0,.075*global.game_spd)
	...
	if sprite_index != spr_bluepomp_knockdown
	{
		sprite_index	= spr_bluepomp_knockdown
		image_index		= 0
	}
}#endregion
...
dommage_system = function() {#region
	...
	&& state != state_door
	{
		enemy_dommage_slash(state != state_knockdown,function(){ #region
			var inst = instance_place(x,y,obj_player_attack)
			
			audio_play_sound(sound_swordclash,999,0)
		...
		enemy_dommage_ball(state != state_knockdown,function(){ #region
		var inst = instance_place(x,y,obj_player_ball)
		if inst
		{
			sleep(12)
			screen_shake_set(6,20,-2)
			
			create_bullet(x,y,depth-1,point_direction(x,y,obj_player.x,obj_player.y),10)

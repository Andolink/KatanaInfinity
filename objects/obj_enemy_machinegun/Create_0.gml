...
state_walk = function() {#region
	...
	enemy_ai_walk(spr_machinegun_turn,spr_machinegun_walk)
...
state_follow = function() {#region
	...
	{
		sprite_index = spr_machinegun_run
		image_speed_set(abs(hspd)/3)
	}
		
	if enemy_ai_saw_player
	&& ((distance_to_object(obj_player)>20&& (distance_to_object(obj_player)<300))
	|| obj_player.state = obj_player.state_hit)
	{state = state_attack}
	else 
	if point_distance(x,y,obj_player.x+obj_player.hspd,obj_player.y)<40
...
state_kick_player = function() {#region
	...
	if sprite_index != spr_machinegun_punch
	{
		image_speed_set(1)
		attack_dir = point_direction(x,y,obj_player.x,obj_player.y)
		attack_in_done = false
		sprite_index	= spr_machinegun_punch
		image_index		= 0
	}
...
state_attack = function() {#region
	...
	if reload_time != 0
	{
		if sprite_index != spr_machinegun_reload
		{
			sprite_index	= spr_machinegun_reload
			image_index		= 0
		}
	}
	else if sprite_index != spr_machinegun_aim
	{
		image_speed_set(0)
		sprite_index	= spr_machinegun_aim
	}
	...
	||  (distance_to_object(obj_player)<20 && obj_player.state != obj_player.state_hit)
	{
		state = state_follow
	}
	else
...
state_hit = function() {#region
	...
	if in_air
	{
		...
		sprite_index = spr_machinegun_hurtfly
		image_speed_set(0)
	}
	else
	{
		...
		{
			sprite_index = spr_machinegun_stairfall
			image_speed_set(1)
		...
		else
		{
			...
			image_speed_set(1)
			if sprite_index != spr_machinegun_hurtground
			{
				...
				image_speed_set(1)
				sprite_index	= spr_machinegun_hurtground
				image_index		= 0
			}
		}
	}
...
state_door = function() {#region
	...
	switch door_step 
	{
		case 0 : 
			...
			sprite_index = spr_machinegun_enterstair
		...
		case 1 : 
			...
			sprite_index = spr_machinegun_leavestair

...
state_walk = function() {#region
	...
	enemy_ai_walk(spr_pomp_turn,spr_pomp_walk)
...
state_follow = function() {#region
	...
	{
		sprite_index = spr_pomp_run
		image_speed_set(abs(hspd)/3)
	}
	
	if point_distance(x,y,obj_player.x+obj_player.hspd,obj_player.y)<60
...
state_attack = function() {#region
	...
	if sprite_index != spr_pomp_attack
	{
		image_speed_set(1)
		attack_dir = point_direction(x,y,target_x,target_y)
		attack_in_done = false
		sprite_index	= spr_pomp_attack
		image_index		= 0
	}
...
state_hit = function() {#region
	...
	else
	{
		...
		else
		{
			...
			image_speed_set(1)
			if sprite_index != spr_pomp_hurtground
			{
				...
				image_speed_set(1)
				sprite_index	= spr_pomp_hurtground
				image_index		= 0
			}
		}
	}
...
state_knockdown = function() {#region
	...
	if sprite_index != spr_pomp_knockdown
	{
		sprite_index	= spr_pomp_knockdown
		image_index		= 0
	}
}#endregion
...
state_door = function() {#region
	...
	switch door_step 
	{
		case 0 : 
			...
			if image_alpha <= 0
			{
				sprite_index = spr_pomp_enterstair
		...
		case 1 : 
			...
			if image_alpha >= 1
			{
				sprite_index = spr_pomp_leavestair
...
dommage_system = function() {#region
	...
	&& state != state_door
	{
		enemy_dommage_slash(state = state_attack,function(){ #region
